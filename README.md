...
- **Searchable, self-describing quality provenance.** Each note's `corti` frontmatter records the Corti
  release, final live-versus-batch path, backend, exact selected ASR/VAD/diarization artifacts, and
  quality-relevant configuration—without credentials, buckets, or private model paths. Current Vagus
  indexes that producer namespace, so `vagus search parakeet` finds transcripts generated by Parakeet.
