...
pub const MAX_GLOBAL_PROVIDER_CALLS: usize = 4;
pub const MAX_PROVIDER_CALLS: usize = 2;
pub const MAX_FINAL_CALLS: usize = 2;
pub const MAX_QUEUED_AD_HOC_QUESTIONS: usize = 8;
pub const MAX_VISIBLE_AD_HOC_EXCHANGES: usize = 20;
...
pub trait CoordinatorClock: ProviderClock {
...
impl LaneFamily {
    ...
    pub const fn of(lane: Lane) -> Self {
        match lane {
            Lane::Live => Self::Live,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct LaneControlDto {
    pub enabled: bool,
    pub revision: u64,
    pub selection: LaneSelectionDto,
}
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct ControlSnapshotDto {
    pub process_epoch: ProcessEpoch,
    pub session_generation: u64,
    pub control_revision: u64,
    pub steering_revision: u64,
    pub bank_revision: u64,
    pub pinned_question_revision: u64,
    pub master_enabled: bool,
    pub egress_acknowledged: bool,
    pub pinned_auto_enabled: bool,
    pub live: LaneControlDto,
    pub final_lane: LaneControlDto,
    pub questions: LaneControlDto,
}
...
pub trait ControlPersistence: Send {
...
pub struct TranscriptWatermark {
    ...
    pub finalized_word_tokens: u64,
    pub covered_speech_ms: u64,
...
pub struct RequestSubmission {
    ...
    pub expected_context_truncated: bool,
    ...
    pub requires_final_recovery: bool,
}
...
pub struct FinalJournalBoundary {
    pub recording_id: String,
    pub request_group_id: corti_postprocess::RequestGroupId,
    pub call_id: CallId,
    pub request_key: RequestKey,
...
pub struct FinalRecoveryRecord {
    pub call_id: CallId,
    pub state: FinalJournalState,
}

pub struct FinalRecoveryHit {
    pub boundary: FinalJournalBoundary,
    pub state: FinalJournalState,
    pub output: ProviderOutput,
    pub terminal: Option<TerminalTelemetryDto>,
}

pub enum FinalRecoveryLookup {
    None,
...
pub enum FinalRecoveryDirective {
    None,
    ResumePrepared {
        call_id: CallId,
    },
...
pub struct TerminalTelemetryDto {
    pub call_id: CallId,
    pub recording_id: String,
    pub request_group_id: corti_postprocess::RequestGroupId,
    pub target_id: Option<corti_postprocess::TargetId>,
    pub lane: Lane,
    pub attempt_no: u64,
    pub fence: RequestFence,
    ...
    pub outcome: TerminalOutcomeDto,
    pub error: Option<ErrorCode>,
    pub provider_request_sent: bool,
    pub late_content_discarded: bool,
...
impl ValidatedOutput {
    pub fn rewritten_rows(&self) -> Option<&[TranscriptRow]> {
        match self {
...
pub struct StoreCommit<'a> {
    pub request_key: RequestKey,
    pub lane: Lane,
    pub local_cache_mode: LocalCacheMode,
    /// Raw typed provider output is persisted only inside the encrypted store and revalidated on every hit.
    pub cache_output: &'a ProviderOutput,
    pub output: &'a ValidatedOutput,
    pub final_boundary: Option<&'a FinalJournalBoundary>,
    pub telemetry: &'a TerminalTelemetryDto,
}
...
pub trait EncryptedPostprocessStore: Send {
...
pub fn provider_support_catalog() -> Vec<ProviderDescriptor> {
    [
...
pub struct ProviderStateDto {
    ...
    pub service_error: Option<ErrorCode>,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize)]
#[serde(rename_all = "snake_case")]
pub enum LaneStateDto {
    Disabled,
    WaitingForPhrase,
    Debouncing,
    Queued,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct LaneStateEventDto {
    pub lane: Lane,
    pub state: LaneStateDto,
    pub code: Option<ErrorCode>,
    pub fence: UiFenceDto,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct NoticeDto {
    pub role: &'static str,
    pub visible_message: &'static str,
    pub episode: u64,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize)]
#[serde(rename_all = "snake_case")]
pub enum AccountingFinalityDto {
    Provisional,
    Final,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct AccountingEventDto {
    pub call_id: CallId,
    pub recording_id: String,
    pub lane: Lane,
    pub fence: RequestFence,
    pub finality: AccountingFinalityDto,
    pub usage: NormalizedUsage,
    pub cost: CostEstimate,
    pub late: bool,
}
...
impl DispatchTicket {
    pub fn request(&self) -> &HostedRequest {
        &self.call.request
    }
...
pub struct ApplyReady {
    ...
    pub journal_call_id: Option<CallId>,
    pub lane: Lane,
    pub fence: RequestFence,
...
#[derive(Debug, Clone)]
pub struct ProviderPrepareSpec {
    pub provider: ProviderId,
    pub transport: TransportId,
    pub scope: ProviderScope,
    ...
    pub credential_override: Option<CredentialState>,
    pub force_refresh: bool,
}

#[derive(Debug, Clone)]
pub struct QueuedProviderPrepare {
    pub call_id: CallId,
    pub spec: ProviderPrepareSpec,
}

#[derive(Debug, Clone)]
pub struct ProviderPreparation {
    pub credential: CredentialState,
    pub projected_credential: CredentialState,
    pub credential_identity: Option<[u8; 32]>,
    pub catalog: Result<ModelCatalog, ErrorCode>,
}
...
pub struct PostprocessCoordinator {
...
impl PostprocessCoordinator {
    ...
    pub fn control_snapshot(&self) -> &ControlSnapshotDto {
        self.control.snapshot()
    }

    pub fn provider_states(&self) -> impl Iterator<Item = &ProviderStateDto> {
        self.provider_states.values()
    }

    pub fn provider_connection_scope(
        &self,
        provider: &ProviderId,
        transport: &TransportId,
    ) -> Option<ProviderScope> {
    ...
    pub fn provider_access(&self) -> Arc<dyn ProviderAccess> {
        self.providers.clone()
    }

    pub fn watermark(&self) -> TranscriptWatermark {
        self.watermark
    }

    pub fn purge_reusable_cache(&mut self) -> Result<usize, ErrorCode> {
    ...
    pub fn sync_provider_credential(
        &mut self,
        provider: &ProviderId,
        transport: &TransportId,
    ) -> Result<ProviderStateDto, ErrorCode> {
    ...
    pub fn take_events(&mut self) -> Vec<CoordinatorEventDto> {
        self.events.drain(..).collect()
    }

    pub fn apply_patch(&mut self, patch: ControlPatch) -> Result<PatchOutcome, ControlError> {
    ...
    pub fn begin_session(&mut self) -> Result<ControlSnapshotDto, ControlError> {
    ...
    pub fn question_summaries(&self) -> Vec<QuestionSummaryDto> {
        self.ad_hoc
            .iter()
    ...
    pub fn question_content(&self, call_id: &CallId) -> Option<QuestionContent<'_>> {
        self.ad_hoc
            .iter()
    ...
    pub fn mark_vertex_token_lost(&mut self) {
    ...
    pub fn invalidate_provider_scope(&mut self, provider: &ProviderId, transport: &TransportId) {
    ...
    pub fn notify_credentials_changed(&mut self, provider: &ProviderId, transport: &TransportId) {
    ...
    pub fn cancel_provider_requests(&mut self, provider: &ProviderId, transport: &TransportId) {
    ...
    pub fn provider_prepare_is_current(&self, request: &QueuedProviderPrepare) -> bool {
        self.queue.iter().any(|queued| {
    ...
    pub fn complete_provider_prepare(
        &mut self,
    ...
    pub fn on_provider_event(&mut self, event: ProviderEvent) -> Result<(), CoordinatorError> {
    ...
    pub fn acknowledge_live_application(
        &mut self,
        call_id: &CallId,
    ...
    pub fn cancel_call(&mut self, call_id: &CallId, reason: CancellationReason) -> bool {
    ...
    pub fn mark_final_applied(&mut self, call_id: &CallId) -> Result<(), CoordinatorError> {
    ...
    pub fn mark_final_group_applied(
        &mut self,
        call_ids: &[CallId],
    ) -> Result<(), CoordinatorError> {
    ...
    pub fn mark_final_checkpointed(&mut self, call_id: &CallId) -> Result<(), CoordinatorError> {
        self.mark_final_group_checkpointed(std::slice::from_ref(call_id))
    }

    pub fn mark_final_group_checkpointed(
        &mut self,
        call_ids: &[CallId],
    ) -> Result<(), CoordinatorError> {
...
impl CoordinatorIngress {
    pub fn bounded(capacity: usize) -> (Self, Receiver<HotPathCommand>) {
        let (sender, receiver) = sync_channel(capacity);
        (Self { sender }, receiver)
    }

    pub fn standard() -> (Self, Receiver<HotPathCommand>) {
        Self::bounded(COORDINATOR_COMMAND_CAPACITY)
    }

    pub fn try_send(&self, command: HotPathCommand) -> Result<(), IngressError> {
    ...
    pub fn barrier(&self) -> Result<(), IngressError> {
...
pub struct BoundedProviderEventSink {
    sender: SyncSender<ProviderEvent>,
}

impl BoundedProviderEventSink {
    pub fn channel() -> (Arc<Self>, Receiver<ProviderEvent>) {
