...
#   - NeMo TitaNet-Large speaker embedding    (~101 MB, the default; far-end diarization only)
#   - Silero VAD                              (~0.6 MB)
#
# Usage: fetch-models.sh [MODEL_DIR]      (default: ~/Library/Caches/corti/models)
# This fetches the DEFAULT embedding (TitaNet-Large); the other English options (wespeaker-resnet34,
# campplus-en) are downloadable from Settings → Models in the app.
# Models are CC-BY-4.0 (Parakeet, pyannote, TitaNet) / Apache-2.0 — see the crate NOTICE.
set -euo pipefail
...
fetch_file "$BASE/speaker-recongition-models/nemo_en_titanet_large.onnx"
